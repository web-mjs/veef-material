...
const setElementProps = (props, element) => {
    ...
    props.map(x => {
        ...
        if(isEvent(key) && !value.trim) {
            ...
            eventHandlerCache.filter(h => h._evName == evName && h._element == element).map(x => 
                element.removeEventListener(evName, x._handlerFn)
            );
            eventHandlerCache.push({_evName: evName, _element: element, _handlerFn: value})
